...
router = APIRouter(prefix="/documents", tags=["documents"], dependencies=[Depends(require_roles("manager", "board_member"))])
...
@router.post("", response_model=Document, dependencies=[Depends(require_roles("manager"))])
...
@router.patch("/{document_id}", response_model=Document, dependencies=[Depends(require_roles("manager"))])
